...
def index(request):
    ...
    if request.POST:    
        with connection.cursor() as cursor:
            cursor.execute("SELECT school_email, password, suspend FROM users WHERE school_email = %s AND password = %s AND suspend = FALSE", [request.POST['school_email'],request.POST['password']])
